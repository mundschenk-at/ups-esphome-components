...
#define USB_CLASS_HID 0x03
                                                    ...
                                                    uint32_t timeout_ms)
        {
            ...
            ESP_LOGD(ESP32_USB_TAG, "HID GET_REPORT: type=0x%02X, id=0x%02X, max_len=%zu - after usb_host_transfer_alloc",
                     report_type, report_id, *data_len);
            ...
            ESP_LOGD(ESP32_USB_TAG, "HID GET_REPORT: type=0x%02X, id=0x%02X, max_len=%zu - after semaphore",
                     report_type, report_id, *data_len);
            ...
            transfer->callback = [](usb_transfer_t *t)
            {
                ...
                ESP_LOGD(ESP32_USB_TAG, "HID GET_REPORT: transfer->callback");
            ...
            ESP_LOGD(ESP32_USB_TAG, "HID GET_REPORT: type=0x%02X, id=0x%02X, max_len=%zu - after usb_host_transfer_submit_control",
                report_type, report_id, *data_len);
            ...
            if (ret == ESP_OK)
            {
                ...
                if (xSemaphoreTake(done_sem, pdMS_TO_TICKS(timeout_ms)) == pdTRUE)
                {
                    ...
                    ESP_LOGD(ESP32_USB_TAG, "HID GET_REPORT: type=0x%02X, id=0x%02X, max_len=%zu - after xSemaphoreTake",
                        report_type, report_id, *data_len);

                    ret = ctx.result;
                    if (ret == ESP_OK && ctx.actual_bytes > sizeof(usb_setup_packet_t))
                    {
                        ESP_LOGD(ESP32_USB_TAG, "HID GET_REPORT: type=0x%02X, id=0x%02X, max_len=%zu - ctx.result ESP_OK",
                                    report_type, report_id, *data_len);

                        size_t data_received = ctx.actual_bytes - sizeof(usb_setup_packet_t);
                        size_t copy_len = std::min(data_received, *data_len);

                        ESP_LOGD(ESP32_USB_TAG, "HID GET_REPORT: type=0x%02X, id=0x%02X, max_len=%zu, copy_len=%zu - before memcpy",
                            report_type, report_id, *data_len, copy_len);
